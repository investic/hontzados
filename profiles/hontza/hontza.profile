...
function _hontza_placeholder_content(&$context) {
  ...
  global $conf;
	$context['message'] = st($global_url);
	$context['message'] = st("maellllll");
	//$contenido = install_node_export_import_from_file('/var/www/hontza1.investic.net/profiles/hontza/node-export_faq.export');
  install_node_export_import_from_file('profiles/hontza/node-export_faq.export');
	
	$context['message'] = st('Installed faq in HONTZA');
}
...
function _hontza_install_menus(&$context) {
  ...
  $menuprimario7 = install_menu_create_menu_item('clasificaciones',   'Clasificaciones',       '', 'primary-links', 0, 9);
