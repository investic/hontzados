...
<div id="node-<?php print $node->nid; ?>" class="node<?php if ($sticky) { print ' sticky'; } ?><?php if (!$status) { print ' node-unpublished'; } ?>">
  ...
  <div id="i-contenedor">
    <?php if($node->body): ?>
      <div class="item-teaser-texto"><?php print drupal_substr(strip_tags($node->body), 0, 250).' ...'; ?></div>
    <?php endif; ?>
    ...
    <?php //Fin de bloque de FiveStar y comentarios ?>
            ...
            <div id="ffc">
                <?php // $regex = "http\:\/\/"; if(preg_match("/^$regex/", $node->feeds_node_item->guid)): ?>
                  <div class="item-fuente">
                    <?php if ($node->feeds_node_item->url):?>
                     <a href="<?php print $node->feeds_node_item->url; ?>" title="Ver fuente original" target= "_blank">Ver fuente original</a>
                    <?php else: ?>
                ...
                <div class="item-fecha">
                  <span class="etiqueta-gris">Fecha captura:</span><?php print format_date($node->created, 'custom','d/m/Y'); ?>
                </div>
                <div class="item-canal">
                  <span class="etiqueta-gris">Canal: </span> <?php print $node->field_item_canal_reference[0]['view']; ?>
                </div>
            </div>
  ...
  <?php if($uri[0] == 'canales'): ?>
    <div class="n-opciones-item"> 
      ...
      <?php //Anadir tag ?>
        ...
        <a href="<?php print '/'.og_get_group_context()->purl.'/node/'.$node->nid.'/tag'?>" title="Etiquetar">Etiquetar</a>
        </div>
      <?php //Fin anadir tag ?>
  
      <?php //Borrar item ?>
        <div class="n-item-borrar">
          <a href="<?php print '/'.og_get_group_context()->purl.'/node/'.$node->nid.'/delete'?>" title="Borrar">Borrar</a>
        </div>
      <?php //Fin borrar item ?>
  
      <?php //Enviar debate ?>
        <div class="n-item-debate">
          <a href="<?php print '/'.og_get_group_context()->purl.'/node/add/debate/'.$node->nid?>" title="Enviar a debate" target= "_blank">Enviar a &Aacute;rea de debate</a>
        </div>
      <?php //Fin enviar debate ?>
  
      <?php //Enviar trabajo ?>
        <div class="n-item-trabajo">
          <a href="<?php print '/'.og_get_group_context()->purl.'/node/add/wiki/'.$node->nid ?>" title="Enviar a trabajo"target= "_blank">Enviar a &Aacute;rea de trabajo</a>
        </div>
  ...
  <?php if ($page == 1): ?>
   <div id="flagtitulo">
    ...
    <?php // $regex = "http\:\/\/"; if(preg_match("/^$regex/", $node->feeds_node_item->guid)): ?>
     <div class="item-fuente">
      <?php if ($node->feeds_node_item->url):?>
        <span class="etiqueta-gris">Fuente:</span> <a href="<?php print $node->feeds_node_item->url; ?>" title="Ver fuente original" target= "_blank">Ver fuente original</a>
      <?php else: ?>
        <span class="etiqueta-gris">Fuente:</span> <a href="<?php print $node->feeds_node_item->guid; ?>" title="Ver fuente original" target= "_blank">Ver fuente original</a>
      <?php endif; ?>
    </div>
    <?php // endif; ?> 
  ...
  <div class="opciones-item">  
    ...
    <?php //Anadir tag ?>
      ...
      <a href="<?php print '/'.$grupo.'/node/'.$node->nid.'/tag'?>" title="Etiquetar">Etiquetar</a>
      </div>
    <?php //Fin anadir tag ?>
